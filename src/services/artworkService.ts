export function generateSvgPoster(opts: { title: string; subtitle: string; accent?: string; theme?: string; statValue?: string; statLabel?: string }) {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="600" height="900" viewBox="0 0 600 900">
    ...
    <text x="48" y="90" fill="${accent}" font-size="16" font-weight="700" font-family="Arial" letter-spacing="3">INSIGHT</text>
    <line x1="48" y1="104" x2="140" y2="104" stroke="${accent}" stroke-width="2.5" opacity="0.6"/>
    ${statValue ? `
    <text x="48" y="500" fill="#ffffff" font-size="96" font-weight="900" font-family="Arial" filter="url(#shadow)">${statValue}</text>
    ${statLabel ? `<text x="48" y="540" fill="${accent}" font-size="20" font-weight="600" font-family="Arial" letter-spacing="1">${statLabel}</text>` : ''}
    ` : ''}
    <text x="48" y="680" fill="#ffffff" font-size="38" font-weight="800" font-family="Arial" filter="url(#shadow)">${title}</text>
    <text x="48" y="730" fill="#94a3b8" font-size="18" font-weight="400" font-family="Arial">${subtitle}</text>
    <rect x="48" y="810" width="504" height="2" rx="1" fill="${accent}" opacity="0.25"/>
    <text x="48" y="845" fill="#475569" font-size="13" font-family="Arial" letter-spacing="1">TRACKT STATS · ${new Date().toLocaleDateString('it-IT')}</text>
  </svg>`;
  return svg;
}
