...
export async function rebuildAdaptiveRow(configId: string) {
  const {  cfg } = await supabase.from('addon_configs').select('*').eq('id', configId).single();
  const {  insight } = await supabase.from('insight_snapshots').select('*').eq('config_id', configId).maybeSingle();
  const {  prefs } = await supabase.from('config_preferences').select('*').eq('config_id', configId).maybeSingle();
  ...
  if (enabled.includes('weekly')) {
    ...
    details.push({ meta_id: id, meta: {
      ...
      videos: [
        ...
        video(`${id}_2`, insight.summary.topWeeklyTitle || 'Nessun titolo dominante', new Date().toISOString(), insight.summary.topWeeklyTitle ? `Hai guardato questo contenuto ${insight.summary.topWeeklyCount} volte nella settimana.` : 'Questa settimana non cՏ ancora un titolo dominante.')
      ]
    }});
  }
  ...
  if (enabled.includes('seasonal')) {
    ...
    cards.push(await cardMeta(id, 'La stagione ti sta cambiando', seasonText, null, null, insight.recurring_titles?.[0]?.title || insight.rewatch_titles?.[0]?.title, insight.seasonal_key === 'christmas' ? '#dc2626' : insight.seasonal_key === 'halloween' ? '#f97316' : insight.seasonal_key === 'summer' ? '#f59e0b' : '#0ea5e9'));
    details.push({ meta_id: id, meta: {
      id,
      type: 'series',
