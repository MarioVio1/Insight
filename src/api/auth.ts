...
router.get('/login/:configId', async (req, res) => {
  return res.redirect(buildAuthorizeUrl(req.params.configId));
});
router.get('/callback', async (req, res) => {
  ...
  await supabase.from('addon_configs').update(payload).eq('id', state);
  await syncConfig(state);
  res.send(`<html><body style="font-family:sans-serif;padding:40px"><h1>Trakt collegato</h1><p>Puoi tornare alla pagina di configurazione.</p><p><a href="/configure/${state}">Apri configurazione</a></p></body></html>`);
});
